...
<bean:define id="defaultPageTitle" value="Unit POC Add/Edit"/>
...
<div class="colmask leftmenu"><div class="colright">
	<div class="col1wrap"><div class="col1">
		...
		<% if (editType.equals("add")) { %>
			<input type="hidden" name="action" value="shipPocAddDo"/>
		<% } else { %>
			<input type="hidden" name="action" value="shipPocEditDo"/>
			<html:hidden name="inputBean" property="shipPocPk"/>
		<% } %>
		<tbody>
			...
			<tr><td class="nobordered" align="left"> <!-- tan_table -->
				...
				<tbody>
					<tr>
						<td class="fieldName">Primary FACET POC:</td>
						<td colspan="5"><html:checkbox name="inputBean" property="isPrimaryPoc" value="Y"/></td>
					</tr>
					<tr>
					...
					<tr>
						<td class="fieldName"><span class="regAsterisk">*</span>Unit:</td>
						<td colspan="3">
							<html:select name="inputBean" property="shipPk">
								<html:option value=""/>
								<html:options collection="shipList" property="shipPk" labelProperty="shipNameTypeHull"/>
							</html:select>
						</td>
						<td class="fieldName">Division:</td>
						<td><html:text name="inputBean" property="dept" size="12" maxlength="50"/></td>
					</tr>
					<tr>
			...
			<tr><td class="nobordered" align="left"> <!-- tan_table -->
				...
				<tbody>
					...
					<tr>
						<td class="fieldName">Alt E-Mail:</td>
						<td colspan="5"><html:text name="inputBean" property="altEmail" size="38" maxlength="255"/></td>
					</tr>
					<tr>
						<td class="fieldName">Work Phone:</td>
						<td><html:text name="inputBean" property="workNumber" size="15" maxlength="25"/></td>
						<td class="fieldName">Cell/Alt Number:</td>
		...
		<table id="borderlessTable" border="0" cellspacing="0"><tbody>
			<tr>
				...
				<td align="center"><input type="button" onclick="window.location='user.do?action=pocList&projectPk=<bean:write name="projectPk"/>';" value="Cancel"/></td>
			</tr>
		</tbody></table>
		</html:form>
		</center>
...
<script type="text/javascript">
	...
	function valFields() {
		...
		var email = stripSpaces(document.userForm.email.value);
		var altEmail = stripSpaces(document.userForm.altEmail.value);
		var shipPk = document.userForm.shipPk.value;

		document.userForm.lastName.value = lastName;
		document.userForm.firstName.value = firstName;
		document.userForm.email.value = email;
		document.userForm.altEmail.value = altEmail;
		...
		if (shipPk.length < 1) {
			...
			document.userForm.shipPk.focus();
			return false;
		} //end of if
