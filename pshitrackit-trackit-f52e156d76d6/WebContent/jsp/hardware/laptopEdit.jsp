...
<bean:define id="defaultPageTitle" value="Laptop Add/Edit"/>
...
<div class="colmask leftmenu"><div class="colright">
    ...
    <tags:projectPkBreadcrumb projectPk="${projectPk}" pageTitle="${pageTitle}" hideOptions="true"
      parentUrl="hardware.do?action=laptopList&projectPk=${projectPk}" parentTitle="Laptop List" />
		...
		<% if (editType.equals("add")) { %>
			<input type="hidden" name="action" value="laptopAddDo"/>
		<% } else { %>
			<input type="hidden" name="action" value="laptopEditDo"/>
		<% } %>
		<html:hidden name="inputBean" property="laptopPk"/>
		<tbody>
			<tr><th>Laptop</th></tr>
			<tr><td class="nobordered" align="left"> <!-- tan_table -->
				<table class="border-zero cellspacing-zero cellpadding-3" width="750">
				<colgroup><col width="100"/><col width="100"/><col width="175"/><col width="100"/><col width="275"/></colgroup>
					<tr>
						<td rowspan="6" class="TOP"><img src="images/hardware/laptop.png" width="100" height="75"/></td>
						...
						<td colspan="5">
							<html:select name="inputBean" property="currProductName" onchange="checkNew(this, 'productName');">
								<logic:present name="productNameList"><html:options name="productNameList"/></logic:present>
					...
					<tr>
						<td class="fieldName" nowrap>Computer Name:</td>
						<td><html:text name="inputBean" property="computerName" size="25" maxlength="50"/></td>
					...
					<tr>
						...
						<td>
							<html:select name="inputBean" property="currModelNumber" onchange="checkNew(this, 'modelNumber');">
								<logic:present name="modelNumberList"><html:options name="modelNumberList"/></logic:present>
								<html:option value="null">Add new...</html:option>
							</html:select>
					...
					<tr>
						<td class="fieldName" nowrap>MAC Address:</td>
						<td><html:text name="inputBean" property="macAddress" size="18" maxlength="17"/></td>
						<td class="fieldName" nowrap>Origin:</td>
						<td>
							<html:select name="inputBean" property="currOrigin" onchange="checkNew(this, 'origin');">
								<logic:present name="originList"><html:options name="originList"/></logic:present>
								<html:option value="null">Add new...</html:option>
							</html:select>
					...
					<tr>
						...
						<td>
							<html:select name="inputBean" property="currCustomer" onchange="checkNew(this, 'customer');">
								...
								<logic:present name="customerList"><html:options name="customerList"/></logic:present>
								<html:option value="null">Add new...</html:option>
							</html:select>
		...
		<table id="borderlessTable" border="0" cellspacing="0"><tbody>
			<tr>
				...
				<td align="center">
					<a class="btn btn-default" href="hardware.do?action=laptopList&projectPk=<bean:write name="projectPk"/>">Cancel</a>
				</td>
			</tr>
		</tbody></table>
		</html:form>
...
<script type="text/javascript">
	...
	$(document).ready(function () {
		if ($('#status').val() == '' || $('#status').val() == '5 - Testing & Prep' || $('#status').val() == '6 - Available & Ready' || $('#status').val() == '8 - Not Returned to PSHI') {
			$('.statusNotesTd').hide();
		} else {
			$('.statusNotesTd').show();
		} //end of else
	...
	function valFields() {
		...
		var macAddress = stripSpaces(document.hardwareForm.macAddress.value);
		...
		document.hardwareForm.macAddress.value = macAddress;
		...
		if (preppedDate.length >= 1 && !validateDate(preppedDate, "Date Prepped")) {
